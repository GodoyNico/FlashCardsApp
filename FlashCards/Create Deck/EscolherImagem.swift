...
class EscolherImagem: NSObject, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    func selecionadorImagem(_ viewController: UIViewController, _ retorno: @escaping ((UIImage) -> ())) {
        ...
        isso serve para o retorno dessa funcao estar em outro metodo, no caso, apos a
        escolha da imagem. */
        retornoSelecionador = retorno;

        /* Declara o viewController como o passado como parametro,
        isso serve para as transicoes de tela.*/
        self.viewController = viewController;

        //Cria uma acao que chama o metodo "openCamera"
        let camera = UIAlertAction(title: "Camera", style: .default) { UIAlertAction in
            
            //Função que sera criada abaixo
            self.abrirCamera()
        }
    ...
    func abrirCamera() {
        ...
        if(UIImagePickerController .isSourceTypeAvailable(.camera)) {
            ...
            //Gera alerta se a pessoa não possue câmera no dispositivo ou
            //caso você rode em um simulador.
    ...
    //Método chamado quando a pessoa escolhe uma imagem
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey : Any]) {
        ...
        retornoSelecionador?(normalizedImage(image:image))
    }
    ...
    func normalizedImage(image: UIImage) -> UIImage {
      ...
      let normalizedImage : UIImage = UIGraphicsGetImageFromCurrentImageContext() ?? image
